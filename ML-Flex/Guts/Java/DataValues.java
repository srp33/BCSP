...
public class DataValues implements Iterable<String>, Comparable
{
    ...
    public void FormatDataPointNames()
    {
        for (String dataPointName : GetDataPointNames())
            UpdateDataPointName(dataPointName, Utilities.FormatName(dataPointName));
    }
    ...
    public String toShortString()
    {
        ...
        if (dataPointsNames.size() > 5)
            dataPointsNames = Lists.Subset(dataPointsNames, 0, 5);
    ...
    public static DataValues FromString(String text)
    {
        ...
        for (String pair : Lists.CreateStringList(parts[1].split(",")))
        {
            String[] pairParts = pair.split("=");
            instance.AddDataPoint(pairParts[0], pairParts[1]);
        }
    ...
    /** Indicates how many data points contain the specified value.
     *
     * @param dataPointNames Data point names to check
     * @param queryValue Query value
     * @return Number of data points that contain the query value
     */
    public int CountMatchingValues(ArrayList<String> dataPointNames, String queryValue)
    {
        int count = 0;

        for (String dataPointName : dataPointNames)
            if (GetDataPointValue(dataPointName).equals(queryValue))
                count++;
