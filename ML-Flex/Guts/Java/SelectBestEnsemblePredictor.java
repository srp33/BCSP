...
/** This class represents an ensemble/combiner approach that computes the combined prediction according to the performance attained in the inner cross-validation folds. The single ModelSelector that attained the best performance in the inner folds is used as the outer-fold combiner prediction.
 * @author Stephen Piccolo
 */
public class SelectBestEnsemblePredictor extends AbstractEnsemblePredictor
{
    @Override
    ...
    private EnsemblePredictionInfo SelectBestPrediction(EnsemblePredictionInfos combinedPredictionInfos) throws Exception
    {
        ...
        {
            if (best == null || info.GetSimpleWeight() > best.GetSimpleWeight())
                best = info;
        }
